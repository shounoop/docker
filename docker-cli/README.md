...
| docker run -p 8080:80 -v [working-directory]:[in-container-path] [image] | Mounts a local directory (working-directory) on your machine to a path inside the container ([in-container-path]).|
| docker run -p 8080:80 -v [working-directory]:[in-container-path] [image] [command] | Runs a specific command inside the container after mounting a local directory.|
